...
- Steam RC verification entrypoint: `npm run verify:steam-rc` runs the current-build static release gates, refreshes the Windows desktop package, runs Electron and packaged-executable smoke, validates live deployment evidence, validates trailer candidate evidence, and writes `test-results/steam-rc-verify-*/report.json`; latest fast same-build pass is `test-results/steam-rc-verify-2026-05-17T20-42-57-204Z/report.json`, covering build, provenance, Steam assets, store metadata, same-build desktop package, audio mix, live deployment, trailer evidence, and release-readiness audit. Latest full browser/playtest pass remains `test-results/steam-rc-verify-2026-05-17T18-05-06-172Z/report.json`. Use `npm run verify:steam-rc -- --full` when browser smoke and the release playtest should be added after the fast same-build package gates.
...
- Latest packaged desktop/live deployment hardening: `npm run desktop:smoke:packaged` now proves the actual packaged `Nova Swarm.exe` can launch and render from `release/desktop/win-unpacked/`, and `npm run check:live-deployment` makes the private-domain deploy a tracked release gate. Latest fast Steam RC verification passed at `test-results/steam-rc-verify-2026-05-17T20-42-57-204Z/report.json`; release audit now has 15 automated passing checks and still blocks only on Steamworks IDs, Steam-client validation, and human approval.
