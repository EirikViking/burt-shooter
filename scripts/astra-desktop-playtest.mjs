...
async function shot(name){await page.screenshot({path:path.join(out,`${name}.png`)});report.checks.push({name,state:await state()});flush();console.log(label,name);}
...
async function record(){
  ...
  report.recording={video,frames:frames.length,elapsedSeconds:frames.at(-1).time-frames[0].time,audio:false,description:'Actual packaged game; normal wall-clock speed, scripted keyboard input, isolated practice run with QA invulnerability.'};flush();
}
try {
  ...
  async function open(params={}){const u=new URL(base);for(const[k,v]of Object.entries({offlineLeaderboard:'1',...params}))u.searchParams.set(k,v);const at=Date.now();await page.goto(u.href,{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>window.__game&&window.render_game_to_text,null,{timeout:120000});return at;}
  ...
  const debug={autostart:'1',debugBossToken:'NOVA_DEBUG_2026','nova-devtools-hash':'f07e7cbbaa835bfa3ecf9bb181e93e59a8f86021ddcda00ec835edcad56a559c'};
  const denseAt=await open({...debug,startLevel:'90'});await ready();report.performance.push({name:'dense-load',loadMs:Date.now()-denseAt});await page.waitForTimeout(7000);await measure('sector-90-dense',20000);await shot('06-dense');
  if(!baseline)await record();
  await open({...debug,startAtBoss:'1',startLevel:'1'});await ready();await page.waitForTimeout(1800);await measure('boss',12000);await shot('07-boss');
  ...
  await open();await page.waitForFunction(()=>window.__game.scenes.menu?.runModeLaunchButton);const b=await page.evaluate(()=>{const b=window.__game.scenes.menu.runModeLaunchButton.getBounds();return{x:b.x+b.width/2,y:b.y+b.height/2};});await page.mouse.click(b.x,b.y);await ready();await shot('11-restarted');
