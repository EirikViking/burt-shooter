...
out=os.path.abspath('docs/astra-v2-models/renders/detonation');os.makedirs(out,exist_ok=True)
...
for i in range(1):
 angle=i*math.tau/12;direction=Vector((math.cos(angle),rng.uniform(-.45,.45),math.sin(angle)*.85)) if i else Vector((0,0,0))
 ...
 modifier=o.modifiers.new('Disrupted gas front','DISPLACE');modifier.texture=texture;modifier.strength=.44;modifier.texture_coords='GLOBAL'
...
for frame in range(start,start+count):
 t=frame/23;expansion=1-math.exp(-t*5.5)
 for i,(o,direction,scale,noise,weight,emission) in enumerate(lobes):
  radius=(.18+expansion*2.5)*scale
  o.location=direction*(expansion*1.9);o.location.z+=t*.30;o.scale=(radius,radius*.85,radius*(1+t*.16));o.rotation_euler=(t*.6+i,t*.35,i*.82)
  noise.inputs['W'].default_value=i*.7+t*1.9
  weight.inputs[1].default_value=22*(1-t*.72)
  emission.inputs[1].default_value=420*max(0,1-t)**2.6*(.7+.3*math.sin(i*1.7+t*2))
 s.render.filepath=os.path.join(out,'%02d.png'%(frame+1))
 if frame==6:bpy.ops.wm.save_as_mainfile(filepath=os.path.abspath('docs/astra-v2-models/detonation-master.blend'))
