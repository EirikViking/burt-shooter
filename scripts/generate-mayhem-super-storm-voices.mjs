...
import {
  ...
  MAYHEM_SUPER_STORM_DEFAULT_VOICE_NAME,
  MAYHEM_SUPER_STORM_MODEL_ID,
  mayhemSuperStormVoiceLines
} from '../src/config/MayhemSuperStormVoiceLines.js';

const apiKey = process.env.ELEVENLABS_API_KEY || process.env.ELEVEN_LABS_API_KEY;
const voiceId = process.env.ELEVENLABS_MAYHEM_SUPER_STORM_VOICE_ID ||
  process.env.ELEVENLABS_MENU_BOSS_VOICE_ID ||
  process.env.ELEVENLABS_BOSS_DEATH_VOICE_ID ||
  MAYHEM_SUPER_STORM_DEFAULT_VOICE_ID;
const modelId = process.env.ELEVENLABS_MAYHEM_SUPER_STORM_MODEL_ID ||
  process.env.ELEVENLABS_MENU_BOSS_MODEL_ID ||
  process.env.ELEVENLABS_MODEL_ID ||
  MAYHEM_SUPER_STORM_MODEL_ID;
const outputDir = path.resolve('public/audio/voice/mayhem-super-storm');
const force = process.argv.includes('--force');
const dryRun = process.argv.includes('--dry-run');
const delayMs = Number(process.env.ELEVENLABS_MAYHEM_SUPER_STORM_DELAY_MS || 725);
...
function requireGenerationInputs() {
  ...
  if (!voiceId) {
    throw new Error('A Mayhem super-storm boss voice id is required.');
  }
}
...
async function tts(line, index, attempt = 1) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`ElevenLabs Mayhem super-storm voice failed for ${line.id}: HTTP ${response.status} ${body.slice(0, 220)}`);
  }
...
async function main() {
  ...
  const selectedLines = onlyIds
    ? mayhemSuperStormVoiceLines.filter((line) => onlyIds.has(line.id) || onlyIds.has(`${line.id}.mp3`))
    : mayhemSuperStormVoiceLines;

  if (onlyIds && selectedLines.length !== onlyIds.size) {
    const known = new Set(mayhemSuperStormVoiceLines.flatMap((line) => [line.id, `${line.id}.mp3`]));
    const unknown = [...onlyIds].filter((id) => !known.has(id));
    throw new Error(`Unknown Mayhem super-storm voice id(s): ${unknown.join(', ')}`);
  }
  ...
  for (let index = 0; index < selectedLines.length; index += 1) {
    ...
    if (dryRun) {
      skipped += 1;
      console.log(`dry-run ${filename}: ${line.text}`);
      continue;
    }

    const sourceIndex = mayhemSuperStormVoiceLines.findIndex((candidate) => candidate.id === line.id);
  ...
  console.log([
    `mayhem super-storm voicepack complete: generated=${generated}`,
    `skipped=${skipped}`,
    `voice=${MAYHEM_SUPER_STORM_DEFAULT_VOICE_NAME}`,
    `dir=${outputDir}`
  ].join(' '));
}
