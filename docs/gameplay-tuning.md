...
- Boss spacing lives in `src/config/BalanceConfig.js` under `difficulty.MIN_WAVES_BETWEEN_BOSSES` and `difficulty.MIN_SECONDS_BETWEEN_BOSSES`. Current defaults are 6 real waves and 75 seconds before a boss gate can open.
- Early waves use six curated waves for levels 1-4, with 46-57 normal enemies before the boss. The goal is more action before pressure spikes.
