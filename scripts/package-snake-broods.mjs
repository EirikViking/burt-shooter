...
const baseline='E:/Codex/builds/nova-swarm/veilborn-test-30/package/win-unpacked';
const root='E:/Codex/builds/nova-swarm/snake-broods/package',dist='E:/Codex/builds/nova-swarm/snake-broods/build-dist';
...
for(const folder of ['art/snake-broods','audio/sfx/snake-broods','audio/voice/mysteries-v2'])
 for(const f of fs.readdirSync('public/'+folder))changed.push({name:'dist/'+folder+'/'+f,src:'public/'+folder+'/'+f});
...
if(!process.env.SNAKE_BROOD_PACKAGE_VERIFY_ONLY){
 if(process.env.SNAKE_BROOD_PACKAGE_REFRESH==='1'){
  const previous=JSON.parse(fs.readFileSync('E:/Codex/builds/nova-swarm/snake-broods/package.json'));
  assert.equal(previous.root,root);assert.equal(previous.baseline,baseline);assert.ok(fs.existsSync(source+'/dist/index.html'));
 }else{
...
fs.writeFileSync('E:/Codex/builds/nova-swarm/snake-broods/package.json',JSON.stringify(receipt,null,2));console.log('PASS',JSON.stringify({...receipt,changed:changed.length}));
