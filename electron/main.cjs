...
let baseUrl = null;
const isolatedUserDataDir = bossEncounterTest
...
if (isolatedUserDataDir) {
  try {
    if (isFreshProfile && !bossEncounterTest && !process.env.NOVA_SWARM_USER_DATA_DIR && fs.existsSync(isolatedUserDataDir)) {
    ...
    if (bossEncounterTest) throw error; // Never continue a test in the real profile.
