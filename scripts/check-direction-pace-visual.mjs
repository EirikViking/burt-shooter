...
try{
 ...
 assert.ok(captures[0].state.length);assert.ok(captures.flatMap(c=>c.state).every(e=>e.duration>=2000));const timeline=await page.evaluate(()=>window.__entryTimeline);assert.ok(timeline.some(s=>s.elapsed>=1600&&s.elapsed<=1900&&s.entry),'Ships still entering around 1.7 seconds; sampled independently of screenshot latency');
 ...
 assert.deepEqual(errors,[]);writeFileSync(`${out}/visual.json`,JSON.stringify({ok:true,errors,captures,timeline,note:'Actual source game, staged wave and attack, isolated QA input; video uses wall-clock animation.'},null,2));await context.close();
