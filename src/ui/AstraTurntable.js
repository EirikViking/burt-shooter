...
export class AstraTurntable extends Container{
 constructor(index,fallback,{idle=false,captionRatio=.018,captionY=.32,highResolution=false}={}){
  ...
  this.views=[new Sprite(fallback),new Sprite(fallback)];this.views.forEach(v=>{v.anchor.set(.5);v.eventMode='none';v.visible=false;this.addChild(v);});this.views[1].alpha=0;
  ...
  this.caption.anchor.set(.5,0);this.caption.y=this.baseSize*captionY;this.caption.eventMode='none';this.caption.visible=false;this.addChild(this.caption);
  ...
  this.solid = new SolidShipView(index, highResolution ? showroomPixels : 768, highResolution ? 'menu' : 'hangar');
  this.promise=this.solid.promise.then(()=>{
   ...
   // Reveal only after the first genuine 3D frame has been rendered.
   this.ready=true;this.update(0);
   this.views[0].visible=true;this.caption.visible=true;
