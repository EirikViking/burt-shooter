...
function material(color, luminous) {
  ...
  if (!materials.has(key)) {
    ...
    for(const [p,v]of [[0,mix(base,0xe2edf0,luminous?.32:.22)],[.055,mix(base,0x839ca7,luminous?.14:.12)],[.35,base],[1,mix(base,0x000308,luminous?.18:.62)]])grad.addColorStop(p,css(v));
    ...
    for(let x=420;x<650;x+=17){c.beginPath();c.moveTo(x,0);c.lineTo(x-110,256);c.stroke();}
...
export function drawAstraPanel(g, x, y, width, height, radius = 7, fillStyle = {}, strokeStyle = {}) {
  ...
  if (width > 42 && height > 25 && alpha > 0.35) {
    ...
    const metal = luminous ? 0xe7fcff : 0x93a7b0;
