...
export function drawColossusWarning(g,b,token,progress) {
  ...
  if(['ring','adds','radial','spiral','clock','chord'].includes(type)||['spiral','clock','chord'].includes(token.attack)){
    ...
    for(const sector of assaultSectors(h)){
      ...
      g.moveTo(...p(a,inner)).lineTo(...p(a,outer)).stroke({color,width:1,alpha:.25});
      ...
      const mid=(a+sector.end)/2,r=inner+(outer-inner)*progress;
      g.moveTo(...p(mid-.025,r-9)).lineTo(...p(mid,r)).lineTo(...p(mid+.025,r-9)).stroke({color,width:2,alpha:.75});
    }
  ...
  if(type==='wall'||token.attack==='wall'){
    ...
    for(const x of b.getWallColumnOffsets()){
      ...
      for(const side of [-1,1]){
        ...
        for(let i=0;i<8;i++){const y=60+((i/8+progress*.5)%1)*(length-60);g.moveTo(x-5,y).lineTo(x,y+10).lineTo(x+5,y).stroke({color:0xffe9c8,width:2,alpha:.7});}
      }
    }
    return true;
  }
