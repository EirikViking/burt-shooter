...
const out=path.resolve('test-results/astra-opening-playthrough');mkdirSync(out,{recursive:true});
...
const app=await electron.launch({executablePath:build.executable,args:['--nova-fresh-profile','--windowed'],env:{...process.env,NOVA_SWARM_USER_DATA_DIR:path.join(out,'profile'),NOVA_SWARM_FRESH_PROFILE:'1',NOVA_SWARM_WINDOWED:'1'},timeout:120000});
