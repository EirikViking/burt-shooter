...
try{
 ...
 assert.ok(captures[0].state.length);assert.ok(captures.flatMap(c=>c.state).every(e=>e.duration>=2800));assert.ok(captures[1].state.some(e=>e.state==='ENTRY'));
...
console.log('PASS visible 2.8-second entrances and outward Sam/Tyrian hazard captures');
