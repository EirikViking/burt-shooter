...
const host = process.env.GAMEOVER_CAPTURE_HOST || '127.0.0.1';
const port = process.env.GAMEOVER_CAPTURE_URL ? null : (Number(process.env.GAMEOVER_CAPTURE_PORT) || await findAvailablePort(4478));
const baseUrl = process.env.GAMEOVER_CAPTURE_URL || `http://${host}:${port}`;
const outputDir = path.resolve(process.env.GAMEOVER_CAPTURE_OUTPUT_DIR || 'release/steam-trailer/gameover');
const viewport = { width: 1920, height: 1080 };
const canvasOutputName = 'nova-swarm-gameover-result-canvas-60fps.webm';
...
async function findAvailablePort(startPort) {
  ...
  throw new Error(`No available game-over capture port found starting at ${startPort}`);
}
...
async function startCanvasRecording(page) {
  return page.evaluate(() => {
    ...
    window.__steamGameOverCanvasChunks = [];
    window.__steamGameOverCanvasRecorder = recorder;
    recorder.ondataavailable = (event) => {
      if (event.data?.size > 0) window.__steamGameOverCanvasChunks.push(event.data);
    };
    recorder.start(250);
...
async function stopCanvasRecording(page, targetPath) {
  const base64 = await page.evaluate(async () => {
    const recorder = window.__steamGameOverCanvasRecorder;
    ...
    const blob = new Blob(window.__steamGameOverCanvasChunks || [], { type: recorder.mimeType || 'video/webm' });
...
try {
  ...
  await page.evaluate(() => {
    ...
    if (typeof scene?.enterRunbackStage === 'function') {
      scene.enterRunbackStage('runback');
    }
  });
  await page.waitForTimeout(450);
  beforeState = await collectState(page);
  await page.screenshot({ path: path.join(outputDir, 'gameover-result-frame.png'), fullPage: true });
...
const report = {
  ...
  outputDir,
  viewport,
  source: 'Built from live Nova Swarm runtime via offline local leaderboard mode and gameOver scene.',
...
writeFileSync(path.join(outputDir, 'gameover-capture-report.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
