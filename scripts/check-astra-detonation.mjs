...
try{
 ...
 const results=await page.evaluate(async(pixiUrl)=>{
  ...
  debrisParent.position.set(18,32);debrisParent.scale.set(.8);
  const canvas=document.createElement('canvas');canvas.width=256;canvas.height=256;
  const body=new PIXI.Sprite(PIXI.Texture.from(canvas));body.anchor.set(.5);body.scale.set(.9);body.rotation=-.13;hullParent.addChild(body);
  const fragments=new HullBreakup(debrisParent),enemy={kind:'boss',body};
  let meshError=0,meshPeak=0;
  Math.random=()=>{calls++;throw Error('Hull breakup consumed gameplay RNG');};
  try{
   fragments.emit(enemy);fragments.emit(enemy);meshPeak=fragments.bossPieces.filter(p=>p.active).length;
   for(const p of fragments.bossPieces){const positions=p.mesh.geometry.positions,uvs=p.mesh.geometry.uvs;for(let i=0;i<positions.length;i+=2){const actual=p.mesh.toGlobal({x:positions[i],y:positions[i+1]});const expected=body.toGlobal({x:(uvs[i]-.5)*256,y:(uvs[i+1]-.5)*256});meshError=Math.max(meshError,Math.hypot(actual.x-expected.x,actual.y-expected.y));}}
   fragments.update(108);const retired=fragments.bossPieces.every(p=>!p.active&&!p.mesh.visible);
   setReducedMotionEnabled(true);fragments.emit({kind:'boss',body});reduced&&=fragments.bossPieces.every(p=>!p.active);cleared&&=retired;
