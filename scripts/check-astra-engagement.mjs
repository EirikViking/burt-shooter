...
result.warmup=await page.evaluate(async()=>{const p=window.__game.scenes.play;await p.shipCatalogReady;await p.prewarmGeneratedEnemyTexturesForLevel(1,{aheadLevels:2});const {getGeneratedEnemyProfilesForLevel}=await import('/src/config/GeneratedEnemyProfiles.js');const level=p.enemyManager.getNormalWaveDifficultyLevel(1),profiles=getGeneratedEnemyProfilesForLevel(level);return {sourceLevel:1,rosterLevel:level,profiles:profiles.length,missing:profiles.filter(x=>!p.preparedRenderTextureKeys.has(`generated_enemy:${x.spriteIndex}`)).map(x=>x.id)};});
assert.equal(result.warmup.rosterLevel,10);assert.deepEqual(result.warmup.missing,[],'Every hull eligible for the actual first-wave tier must be GPU-prepared');
