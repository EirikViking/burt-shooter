...
const child=spawn(executable||path.resolve('node_modules/electron/dist/electron.exe'),[...(executable?[]:[path.resolve('electron/main.cjs')]),`--${mode}`,'--nova-fresh-profile','--disable-backgrounding-occluded-windows','--disable-renderer-backgrounding','--disable-features=CalculateNativeWinOcclusion'],{cwd:process.cwd(),env,windowsHide:true,stdio:['ignore',log,log]});
