...
export const BONUS_DRONES = Object.freeze([
  ...['Scout','Courier','Skimmer','Warden'].map((name,index)=>Object.freeze({id:`bonus_drone_legacy_${index}`,name, index,textureIndex:index,score:500,color:0xffa451,art:`/art/bonus-drones-20260908/codex/${String(index+1).padStart(2,'0')}.webp`,legacy:true})),
  ...names.map((name,index)=>Object.freeze({id:`bonus_drone_${name.toLowerCase().replaceAll(' ','_')}`,name,index:index+4,textureIndex:index+12,score:650+index*100,color:colors[index],art:`/art/bonus-drones-20260908/codex/${String(index+5).padStart(2,'0')}.webp`,legacy:false}))
]);
