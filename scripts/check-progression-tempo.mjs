...
assert(wavePlan.every((level) => level.waveCount <= 2), 'Levels 1-10 should stay at two normal waves or less before each boss.', errors);
assert(totalNormalWavesToLevel10 <= 20, `Too many normal waves before level 10 (${totalNormalWavesToLevel10}).`, errors);
assert(totalEnemiesToLevel10 <= 120, `Too many normal enemies before level 10 (${totalEnemiesToLevel10}).`, errors);
assert(wavePlan[9].bossHealth <= 250, `Level 10 boss health is too slow for the target tempo (${wavePlan[9].bossHealth}).`, errors);
assert(diff.waveDelayMs <= 1000, `Between-wave briefing is too long (${diff.waveDelayMs}ms).`, errors);
assert(diff.waveCleanupMs <= 1000, `Wave cleanup window is too long (${diff.waveCleanupMs}ms).`, errors);
assert(diff.bossGateMs <= 700, `Boss gate is too long (${diff.bossGateMs}ms).`, errors);
