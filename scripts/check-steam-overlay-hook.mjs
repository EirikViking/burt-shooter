...
function forbidText(relativePath, needle, label) {
  const ok = !read(relativePath).includes(needle);
  if (!ok) errors.push(`${label} still present in ${relativePath}`);
  return { label, file: relativePath, ok };
}
...
const checks = [
  ...
  requireText('electron/main.cjs', 'steamLeaderboardBridge.enableElectronScreenshotCapture(win', 'on-demand Steam screenshot capture enabled'),
  requireText('electron/main.cjs', '--steam-capture-probe', 'repeatable Steam capture runtime probe'),
  requireText('electron/steamLeaderboardBridge.cjs', "input?.key !== 'F12'", 'scoped physical F12 handler'),
  requireText('electron/steamLeaderboardBridge.cjs', 'browserWindow.webContents.capturePage()', 'one-shot Electron frame capture'),
  requireText('electron/steamLeaderboardBridge.cjs', 'this.steam.screenshots.addScreenshotToLibrary(', 'one-shot Steam screenshot library handoff'),
  requireText('electron/steamLeaderboardBridge.cjs', 'screenshots.hookScreenshots(true);', 'Steam screenshot hook mode'),
  requireText('electron/steamLeaderboardBridge.cjs', 'continuousMirror: false', 'continuous frame mirror disabled'),
  forbidText('electron/main.cjs', 'attachElectronCaptureSurface', 'automatic continuous capture-surface attachment'),
  forbidText('electron/steamLeaderboardBridge.cjs', 'addElectronSteamOverlay', 'continuous native frame mirroring'),
...
const f12Checks = f12SourceFiles.map((file) => {
  ...
  if (claimed) errors.push(`F12 is claimed outside the dedicated on-demand screenshot bridge by ${file}`);
  return { label: 'F12 unclaimed outside screenshot bridge', file, ok: !claimed };
});
...
for (const file of packagedFiles) {
  if (!file.exists) errors.push(`missing packaged Steam runtime prerequisite: ${file.path}`);
}
...
warnings.push('True Shift+Tab, physical F12, and fresh-frame Steam Game Recording playback still require the Steam-client manual QA procedure in docs/qa/steam-capture-manual.md.');
...
const report = {
  ...
  steamScreenshotHotkey: {
    key: 'F12',
    dedicatedBridgeClaimsHotkey: true,
    claimedOutsideDedicatedBridge: f12Checks.some((check) => !check.ok),
    checks: f12Checks,
    conclusion: f12Checks.every((check) => check.ok)
      ? 'Only the dedicated Electron bridge claims F12, captures one frame, and hands the PNG to ISteamScreenshots.'
      : 'At least one unrelated input surface also claims F12; screenshot behavior is at risk.'
  },
  packagedFiles,
  packagedSmoke,
  overlayConclusion: errors.length === 0
    ? 'The Electron runtime has no continuous full-frame mirror. F12 performs one Electron capture and adds that PNG to the Steam screenshot library. Manual physical-hotkey and fresh-frame Game Recording playback verification remains required.'
    : 'On-demand capture prerequisites are incomplete or unproven.',
  fixedInCode: true,
...
console.log(`[steam-overlay-hook] PASS continuous_mirror=false manual_steam_capture_required=true report=${rel(reportPath)}`);
